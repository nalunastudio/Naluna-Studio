...
const CONSENT_POLICY_VERSION = '2026-09-11-v6';
// Text localizat pentru custom_text.terms_of_service_acceptance (Stripe Checkout) — parafraza
// fidela a refund.html Sectiunea 2 (cerere explicita de incepere imediata a crearii + pierderea
// dreptului legal de 14 zile de anulare odata ce crearea a inceput + fara rambursare pentru
// schimbarea parerii dupa livrare) si a exceptiilor din Sectiunea 3/4 (defect/neconform/nelivrat
// din vina Naluna). Linkuri Markdown absolute (${DOMAIN}/terms.html, ${DOMAIN}/refund.html) —
// pagina Stripe e găzduită separat de domeniul Naluna, linkurile relative nu ar functiona acolo.
// Sub limita reala Stripe de 1200 caractere pentru custom_text (verificat, maximum 653 caractere
// inainte de expandarea ${DOMAIN}).
const CONSENT_TOS_TEXT = {
  ro: `Prin continuare, îi cer Naluna Studio să înceapă imediat crearea melodiei/videoclipului meu personalizat și înțeleg că îmi pierd dreptul legal de anulare de 14 zile odată ce a început crearea. După livrare, nu am dreptul la o rambursare doar pentru că m-am răzgândit sau nu-mi place rezultatul creativ. Acest lucru nu îmi afectează drepturile dacă produsul este defect, nu corespunde descrierii sau nu este livrat din vina Naluna. Vezi [Termenii](${DOMAIN}/terms.html) și [Politica de anulare și rambursare](${DOMAIN}/refund.html).`,
  en: `By proceeding, I ask Naluna Studio to begin creating my personalised song/video immediately, and I understand that I lose my statutory 14-day right to cancel once creation has started. Once delivered, I'm not entitled to a refund just because I change my mind or dislike the creative result. This doesn't affect my rights if the content is faulty, not as described, or undelivered due to a fault on Naluna's side. See [Terms](${DOMAIN}/terms.html) and [Cancellation & Refund Policy](${DOMAIN}/refund.html).`,
  de: `Mit meinem Fortfahren bitte ich Naluna Studio, mein personalisiertes Lied/Video sofort zu erstellen, und mir ist bewusst, dass ich mein gesetzliches 14-tägiges Widerrufsrecht verliere, sobald die Erstellung begonnen hat. Nach der Lieferung habe ich keinen Anspruch auf Rückerstattung, nur weil ich es mir anders überlegt habe oder mir das kreative Ergebnis nicht gefällt. Dies berührt nicht meine Rechte, falls der Inhalt mangelhaft, nicht wie beschrieben oder aufgrund eines Fehlers von Naluna nicht geliefert ist. Siehe [AGB](${DOMAIN}/terms.html) und [Stornierungs- und Rückerstattungsrichtlinie](${DOMAIN}/refund.html).`,
  es: `Al continuar, le pido a Naluna Studio que comience a crear mi canción/vídeo personalizado de inmediato, y entiendo que pierdo mi derecho legal de desistimiento de 14 días en cuanto comienza la creación. Una vez entregado, no tengo derecho a un reembolso solo porque cambie de opinión o no me guste el resultado creativo. Esto no afecta a mis derechos si el contenido es defectuoso, no se corresponde con la descripción o no se entrega por un fallo de Naluna. Consulta los [Términos](${DOMAIN}/terms.html) y la [Política de cancelación y reembolso](${DOMAIN}/refund.html).`,
  it: `Procedendo, chiedo a Naluna Studio di iniziare subito a creare la mia canzone/video personalizzato, e comprendo che perdo il diritto di recesso legale di 14 giorni non appena inizia la creazione. Una volta consegnato, non ho diritto a un rimborso solo perché cambio idea o non mi piace il risultato creativo. Questo non incide sui miei diritti se il contenuto è difettoso, non conforme alla descrizione o non consegnato per un errore di Naluna. Consulta i [Termini](${DOMAIN}/terms.html) e la [Politica di cancellazione e rimborso](${DOMAIN}/refund.html).`,
  fr: `En continuant, je demande à Naluna Studio de commencer immédiatement à créer ma chanson/vidéo personnalisée, et je comprends que je perds mon droit légal de rétractation de 14 jours dès que la création a commencé. Une fois livré, je n'ai pas droit à un remboursement simplement parce que j'ai changé d'avis ou que le résultat créatif ne me plaît pas. Cela n'affecte pas mes droits si le contenu est défectueux, non conforme à la description, ou non livré en raison d'une défaillance de Naluna. Voir les [Conditions](${DOMAIN}/terms.html) et la [Politique d'annulation et de remboursement](${DOMAIN}/refund.html).`,
  bg: `С продължаването моля Naluna Studio да започне незабавно да създава моята персонализирана песен/видео и разбирам, че губя законното си право на отказ от 14 дни, след като създаването е започнало. След доставката нямам право на възстановяване само защото съм размислил/а или не харесвам творческия резултат. Това не засяга правата ми, ако съдържанието е дефектно, не отговаря на описанието или не е доставено поради грешка на Naluna. Виж [Общите условия](${DOMAIN}/terms.html) и [Политиката за анулиране и възстановяване](${DOMAIN}/refund.html).`,
  tr: `Devam ederek Naluna Studio'dan kişiselleştirilmiş şarkımı/videomu hemen oluşturmaya başlamasını istiyorum ve oluşturma başladıktan sonra yasal 14 günlük cayma hakkımı kaybettiğimi anlıyorum. Teslimattan sonra, sadece fikrimi değiştirdiğim veya yaratıcı sonucu beğenmediğim için iade hakkım yoktur. Bu, içerik kusurluysa, açıklamaya uygun değilse veya Naluna kaynaklı bir hata nedeniyle teslim edilmemişse haklarımı etkilemez. Bkz. [Şartlar](${DOMAIN}/terms.html) ve [İptal ve İade Politikası](${DOMAIN}/refund.html).`
};

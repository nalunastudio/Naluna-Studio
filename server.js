...
// de conținut.
// CORECȚIE (2026-09-11, v5->v6, acuratețe legală): textul v5 (mai sus) spunea "deja creat...
// pierd dreptul de 14 zile odata ce incepe LIVRAREA" — framing preluat din vechiul consent_text,
// NEVERIFICAT direct contra textului REAL, live, al terms.html/refund.html. Verificat acum
// exhaustiv: refund.html Sectiunea 2 spune explicit "you lose your statutory 14-day right to
// cancel once CREATION has started" (nu livrarea) si terms.html Sectiunea 3/4 confirma "Creation
// begins immediately after your payment is confirmed". "14 zile" RAMANE (e real, documentat
// explicit in refund.html) — dar declanșatorul corect e inceperea CREARII, nu a livrarii. Textul
// de mai jos a fost rescris sa reflecte STRICT ce spun documentele live acum, nu framing-ul
// "deja creat" mostenit dintr-o corectie anterioara (v4) care a atins doar consent_text/emailul
// de livrare, NICIODATA terms.html/refund.html insele — terms.html/refund.html NU au fost
// modificate aici (in afara scopului acestei corectii).
// CORECȚIE (2026-09-12, v6->v7): textul v6 era corect ca substanță, dar prea lung și afișa
// explicit "14 zile" — decizie de business: mențiunea vizibilă a cifrei exacte se elimină din
// textul de checkout (nu din terms.html/refund.html, care rămân neatinse și în vigoare cu acea
// cifră reală). Substanța legală rămâne EXACT aceeași (crearea începe imediat, fără anulare doar
...
// Secțiunea 3-4 (defect/neconform/nelivrat din vina Naluna). Linkuri Markdown absolute
...
app.post('/api/admin/orders/:orderId/retry-extras', async (req, res, next) => {
  try {
    ...
    const order = await db.getOrderById(req.params.orderId);
    if (!order) return res.status(404).json({ error: 'Comanda nu există.' });
    // BUGFIX (2026-09-25, investigatie "Retry Extras" fals-pozitiv pe comenzi FR/premium
    // neplatite): 'preview_ready' e un status LEGITIM pentru retry in doua cazuri complet
    // diferite — (1) pachetul "video", care randeaza INAINTE de plata prin design (vezi
    // comentariul RELANSARE 2026-08-06 de la POST .../create-video) — statusul singur e
    // suficient acolo; (2) o comanda premium/video DEJA PLATITA a carei editare/regenerare a
    // esuat si a revenit la 'preview_ready' PASTRAND paid_at (vezi markGenerationFailed).
    // Insa un premium 'preview_ready' cu paid_at NULL inseamna STRICT ca nu s-a platit
    // niciodata — extrasele (WAV) nu trebuie niciodata generate inainte de plata (vezi
    // comentariul de la generatePremiumExtras/processConfirmedPayment). Statusul singur nu
    // distinge cele doua cazuri — semnalul real e paid_at, nu status.
    const extrasEligible = order.status === 'ready' || (order.status === 'preview_ready' && (order.plan === 'video' || !!order.paidAt));
    if (!extrasEligible) return res.status(400).json({ error: 'Comanda nu e încă plătită.' });
...
app.get('/api/orders/:orderId', async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      // alese ca bonus surpriza — expus STRICT dupa plata (order.status==='ready'), niciodata
      // inainte. Coloana insasi e oricum NULL pana la confirmarea platii (vezi
      // recordPaidOrderAtomically in db.js), dar gate-ul explicit de aici garanteaza asta
...
// CERINTA (2026-09-13, runda 2, "a treia melodie cadou pentru Premium"): fisierul complet al
// CELEI DE-A TREIA melodii — STRICT Premium, STRICT surpriza post-plata, aleasa RANDOM dintre
// variantele NESELECTATE de client (vezi pickPremiumBonusVariantId, apelata O SINGURA DATA la
// confirmarea platii — lib/entitlements.js + recordPaidOrderAtomically in db.js). ACELASI tipar
// de securitate ca /gift de mai sus (token timing-safe, status==='ready', acces gazduit
// neexpirat) — nicio slabire a controalelor existente. Livrat STRICT la Premium; alte pachete
// nu au niciodata order.premiumBonusVariantId setat, deci getPremiumBonusVariant() returneaza
// null si cererea e refuzata generic, identic cu "fisier indisponibil".
app.get('/media/full/:orderId/bonus', async (req, res, next) => {
  try {
    ...
    const bonusVariant = getPremiumBonusVariant(order);

    if (storage.CLOUD_ENABLED && bonusVariant && bonusVariant.fullKey) {
      const signedUrl = await storage.getSignedDownloadUrl(bonusVariant.fullKey, 600, attachmentDisposition(`melodie-cadou-pentru-${order.recipient}`, 'mp3'));
      return res.redirect(302, signedUrl);
    }

    const filePath = bonusVariant ? path.join(MEDIA_FULL_DIR, `${order.id}-${bonusVariant.id}.mp3`) : null;
    if (!filePath || !fs.existsSync(filePath)) return res.status(404).send('Fișier indisponibil');
    res.download(filePath, `cantec-cadou-${order.recipient}.mp3`);
  } catch (err) {
    next(err);
...
const GENRE_STYLE_MAP = {
  ...
  // 'club hip-hop' nu a functionat" — aprobata explicit, scop STRICT limitat la aceasta valoare):
  // inlocuieste versiunea anterioara ("early-2000s club hip-hop...") cu directia street hip-hop/
  // rap ceruta — "dry snare"/"sparse dark gritty beat" (nu "confident"/"club") resping activ
  // caracterul de club/petrecere; beat-ul descris ca prezent "from the first beat" — caracteristica
  // a INTREGII melodii (ca toate celelalte descrieri de gen), NICIODATA o regula separata de
  // intro. Fara nume de artist/titlu de piesa (verificat, vezi test/hiphop-genre-redefinition.
  // test.js) — STRICT caracteristici muzicale generale. Mai scurta decat versiunea anterioara
  ...
  //
  // NOTA (2026-09-25, investigatie regresie linii lungi in versuri manele_suflet): incercare
  // RESPINSA, gasita prin testare directa — un ADAOS aici (in styleTags, parte din headPart1,
  // NICIODATA scurtata de cascada din buildPrompt()) parea cea mai "sigura" plasare la prima
  // vedere, dar masurat direct impotriva celui mai strans scenariu deja documentat (nunta, nume
  // compuse foarte lungi, relatie foarte lunga, duet — test/manele-suflet-short-intro.test.js
  // #11c) chiar si un adaos de 5 caractere impinge acel scenariu peste marginea unde povestea
  // clientului DISPARE COMPLET din prompt (marja reala inainte de aceasta investigatie: 4
  // caractere, nu 22 cum s-a presupus initial la rescrierea din 2026-09-24). Fix-ul real
  // (instructiune de linii scurte pentru manele_suflet) foloseste in schimb un toggle propriu in
  // cascada de scurtare a lui buildPrompt() (maneleSufletLineLengthClause/
  // includeManeleSufletLineLengthHint) — participa la ACELASI mecanism care protejeaza deja
...
function buildPrompt(order, feedback, genreOverride) {
  ...
  // TASK (2026-09-25, regresie versuri manele_suflet — cerinta explicita, comparatie directa intre
  // doua generari reale ale aceluiasi client): versiunea PLACUTA avea linii scurte, naturale, usor
  // de cantat (4-6 cuvinte/linie, verificat direct pe exemplele reale primite); versiunea NEPLACUTA
  // a produs linii vizibil mai lungi, descriptive (7-8 cuvinte/linie). AUDIT premergator (facut
  // inainte de aceasta modificare, cerinta explicita "nu presupune cauza"): buildPrompt() foloseste
...
async function sendDeliveryEmail(order) {
  ...
  // CERINTA (2026-09-13, runda 2, "a treia melodie cadou pentru Premium"): STRICT Premium — o a
  // treia melodie, aleasa RANDOM la confirmarea platii dintre variantele NESELECTATE de client
  // (vezi pickPremiumBonusVariantId in lib/entitlements.js). Distincta de giftLine de mai sus
  // (care, la Premium, e a DOUA melodie CUMPARATA, niciodata un cadou real) — de aceea foloseste
  // simbolul 🎁 (surpriza), nu 🎵 (continut cumparat), si un text explicit de "cadou"/"surpriza".
  const premiumBonusVariant = getPremiumBonusVariant(order);
  const hasPremiumBonus = !!(premiumBonusVariant && premiumBonusVariant.fullKey);
  const premiumBonusUrl = `${DOMAIN}/media/full/${order.id}/bonus?token=${order.accessToken}`;
  const PREMIUM_BONUS_LINE = {
    ro: `<p>🎁 <a href="${premiumBonusUrl}">Descarcă melodia surpriză</a>, cadou din partea noastră.</p>`,
    en: `<p>🎁 <a href="${premiumBonusUrl}">Download your surprise song</a>, a gift from us.</p>`,
    de: `<p>🎁 <a href="${premiumBonusUrl}">Lade dein Überraschungslied herunter</a>, ein Geschenk von uns.</p>`,
    es: `<p>🎁 <a href="${premiumBonusUrl}">Descarga tu canción sorpresa</a>, un regalo de nuestra parte.</p>`,
    it: `<p>🎁 <a href="${premiumBonusUrl}">Scarica la tua canzone sorpresa</a>, un regalo da parte nostra.</p>`,
    fr: `<p>🎁 <a href="${premiumBonusUrl}">Téléchargez votre chanson surprise</a>, un cadeau de notre part.</p>`,
    bg: `<p>🎁 <a href="${premiumBonusUrl}">Изтегли своята песен изненада</a>, подарък от нас.</p>`,
    tr: `<p>🎁 <a href="${premiumBonusUrl}">Sürpriz şarkınızı indirin</a>, bizden bir hediye.</p>`
  };
  const premiumBonusLine = hasPremiumBonus ? (PREMIUM_BONUS_LINE[order.lang] || PREMIUM_BONUS_LINE.ro) : '';

...
// ============================================================================
// INTRO CADOU VIDEO (2026-09-14): "de la 0:00 pana la intrarea reala a vocii, foloseste O
// SINGURA secventa video a clientului, nu rotatia normala; exact cand incepe vocea, montajul
// normal continua identic". Aplicata STRICT ca o transformare POST-HOC pe planul de cadre DEJA
// complet construit de buildShotPlan() de mai sus (apelat NESCHIMBAT, cu aceiasi parametri ca
// inainte) — NICIODATA o cascada/arhitectura noua de planificare. Motivul e simplu si verificabil
// direct: concatWithCrossfades()/concatWithCrossfadesAndMux() (server.js) citesc STRICT
// shot.duration/transitionOut/transitionDuration din fiecare element — start/end/sectionType/
// kenBurns sunt informative, folosite doar la CONSTRUCTIA planului (deja terminata la acest
// punct). Inlocuind DOAR cadrele de la inceput (indicii < boundaryIdx) cu UN SINGUR cadru nou —
// si pastrand shots[boundaryIdx..] STRICT identice, ca obiecte, cu planul original — garanteaza
// matematic ca randarea/concatenarea de dupa acel punct e IDENTICA cu ce ar fi produs codul
// existent, fara aceasta modificare.
//
// `vocalOnsetSeconds`: timpul REAL (secunde) al primului cuvant cantat cu adevarat, derivat din
// alignedWords (acelasi concept "primul cuvant real" deja folosit de getPreviewStartFromLyrics,
// server.js — reutilizat, nu reimplementat separat). NU un timp fix.
// `mediaItems`: ACEEASI lista `ordered` (deja sortata de sortMediaBySection) folosita de
// buildShotPlan() pentru acest plan — shot.itemIndex indexeaza STRICT in aceasta lista.
//
// FALLBACK (comportamentul ACTUAL, neschimbat), aplicat daca ORICARE dintre conditii nu e
// indeplinita — niciodata un comportament nou degradat:
//   - lipseste orice video incarcat de client (niciun item cu type==='video');
//   - vocalOnsetSeconds nu e un numar finit, pozitiv (aliniere indisponibila/nesigura);
//   - nu exista niciun cadru existent al carui `start` sa ajunga la/dupa vocalOnsetSeconds
//     (limita nu poate fi gasita fiabil in planul curent — inclusiv planuri cu un singur cadru).
// In toate aceste cazuri, planul e returnat NESCHIMBAT (aceeasi referinta, chiar).
//
// Alegerea limitei: primul cadru existent a carui `start >= vocalOnsetSeconds` — NICIODATA mai
// devreme — garanteaza ca noua secventa unica acopera intreg intervalul instrumental (poate fi
// cu putin mai lunga decat vocalOnsetSeconds exact, niciodata mai scurta), evitand sa arate
// montajul normal (taieturi rapide) inca in timp ce melodia e inca instrumentala.
//
// CORECȚIE (2026-09-14, "video-ul de 2 secunde a fost ales pentru intro si s-a repetat in
// bucla"): alegerea VIDEO-ULUI folosit pentru intro (nu limita/momentul intro-ului, neschimbate)
// — daca exista MAI MULTE video-uri incarcate, se alege cel cu durata REALA cea mai mare, nu
// primul din ordine. `videoDurationsByIndex` (optional): array/obiect indexat STRICT ca
// `mediaItems`, cu durata reala (secunde) a fiecarui item VIDEO, ACOLO UNDE e deja cunoscuta —
// apelantul (buildMemoryBackground, server.js) o calculeaza STRICT cand exista 2+ video-uri
// candidate (niciun probe suplimentar pentru cazul obisnuit, un singur video). Lipsa completa a
// acestui parametru (undefined/null), sau lipsa/invaliditatea duratei pentru toate video-urile,
// revine la comportamentul ORIGINAL — primul video eligibil din `mediaItems` — niciodata o
// eroare. La durate egale, castiga primul din ordine (tie-break determinist, fara randomizare).
function selectVideoGiftIntroItemIndex(mediaItems, videoDurationsByIndex) {
  const videoIndexes = (mediaItems || [])
    .map((m, i) => (m && m.type === 'video') ? i : -1)
    .filter(i => i !== -1);
  if (videoIndexes.length === 0) return -1;
  if (!videoDurationsByIndex) return videoIndexes[0];

  let bestIndex = -1;
  let bestDuration = -1;
  for (const i of videoIndexes) {
    const d = videoDurationsByIndex[i];
    if (typeof d === 'number' && Number.isFinite(d) && d > bestDuration) {
      bestDuration = d;
      bestIndex = i;
    }
  }
  return bestIndex !== -1 ? bestIndex : videoIndexes[0];
}

function applyVideoGiftIntro(shots, mediaItems, vocalOnsetSeconds, videoDurationsByIndex) {
  if (!Array.isArray(shots) || shots.length < 2) return shots;
  if (typeof vocalOnsetSeconds !== 'number' || !Number.isFinite(vocalOnsetSeconds) || vocalOnsetSeconds <= 0) return shots;

  const introItemIndex = selectVideoGiftIntroItemIndex(mediaItems, videoDurationsByIndex);
  if (introItemIndex === -1) return shots;

  const boundaryIdx = shots.findIndex(s => s.start >= vocalOnsetSeconds);
  if (boundaryIdx <= 0) return shots; // nimic de inlocuit (vocea incepe deja in primul cadru) sau limita negasita

  const before = shots[boundaryIdx - 1];
  const introShot = {
    start: 0,
    end: shots[boundaryIdx].start,
    duration: shots[boundaryIdx].start,
    energy: 'calm',
    sectionType: shots[0].sectionType,
    itemIndex: introItemIndex,
    occurrence: 0,
    kenBurns: null, // video, niciodata Ken Burns (rezervat exclusiv pozelor — vezi renderShot)
    // Tranzitia CATRE primul cadru pastrat trebuie sa ramana EXACT cea originala — shots[boundaryIdx]
    // isi calculase deja start/end presupunand EXACT aceasta durata de tranzitie (compensatia din
    // buildShotPlan, PASUL 7 de mai sus) — refolosind-o aici, nu strica nimic din timing-ul deja fixat.
    transitionOut: before.transitionOut,
    transitionDuration: before.transitionDuration
  };

  return [introShot, ...shots.slice(boundaryIdx)];
}

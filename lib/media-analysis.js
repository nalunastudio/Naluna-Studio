...
// Ajusteaza granitele INTERIOARE ale planului de cadre deja calculat (shots[i].end ===
// shots[i+1].start) la cel mai apropiat impuls detectat, DOAR daca exista unul suficient de
// aproape (<= maxSnapSeconds) — NICIODATA prima granita (inceputul primului cadru, mereu 0) sau
// ultima (finalul ultimului cadru, mereu durationSeconds), si NICIODATA dincolo de un cadru
// vecin (pastreaza SHOT_PLAN_MIN_SHOT_SECONDS ca durata minima pe ambele parti) — numarul total
// de cadre si durata totala raman EXACT neschimbate, doar POZITIA taieturii se muta usor pe
// impuls. Fara impulsuri (melodie fara analiza reusita, sau nicio potrivire suficient de
// aproape), planul ramane STRICT cel calculat de durate/sectiuni — acesta e fallback-ul.
function snapShotBoundariesToOnsets(shots, onsetTimes, maxSnapSeconds) {
  ...
  for (let i = 0; i < shots.length - 1; i++) {
    const snapped = nearestOnsetWithin(shots[i].end);
    if (snapped === null) continue;
    const minBoundary = shots[i].start + SHOT_PLAN_MIN_SHOT_SECONDS;
    const maxBoundary = shots[i + 1].end - SHOT_PLAN_MIN_SHOT_SECONDS;
    if (maxBoundary <= minBoundary) continue; // cadre deja la durata minima — nu risca suprapunere
    const clamped = Math.min(Math.max(snapped, minBoundary), maxBoundary);
    shots[i].end = clamped;
    shots[i].duration = shots[i].end - shots[i].start;
    shots[i + 1].start = clamped;
...
function buildShotPlan(mediaItems, durationSeconds, sectionTimings, xfadeSeconds, onsetTimes) {
  const n = mediaItems.length;
  ...
  // ALINIERE LA IMPULS (2026-08-24) — vezi comentariul detaliat de la snapShotBoundariesToOnsets
  // mai sus. Aplicata AICI, DUPA compensatia tehnica de mai sus — momentele start/end de la acest
  // punct sunt cele care ajung EFECTIV in fisierul video final (audio-ul melodiei NU e intins,
  // doar segmentele vizuale se suprapun la tranzitie — deci timpul "final" de aici corespunde 1:1
  // cu timpul REAL din melodie, exact ce trebuie aliniat la impuls). Aplicata INAINTE de
  // compensatie, granitele brute nu ar corespunde deloc cu momentele reale din melodie (verificat
  // direct — compensatia cumulativa pe un plan cu multe cadre scurte poate insemna cateva secunde
  // de decalaj pana la mijlocul melodiei, nu o ajustare mica). Fara onsetTimes (analiza audio
  // indisponibila/esuata), shots ramane NESCHIMBAT — exact planul bazat pe sectiuni/durate de mai
  // sus, fallback-ul explicit cerut.
  if (onsetTimes && onsetTimes.length > 0) {
    snapShotBoundariesToOnsets(shots, onsetTimes);
  }

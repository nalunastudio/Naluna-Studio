...
//
// CORECȚIE (2026-09-13, "livrare Standard/Premium/Video dupa plata"): decizia de mai jos
// (HOTFIX 2026-08-08) a fost revizuita explicit — Standard livreaza ACUM si el "melodia
// cadou" (varianta nealeasa), EXACT ca Video, STRICT cand exista o editare REALA (original +
// editat). Un client Standard care nu a folosit niciodata editarea gratuita are o singura
// varianta si nu primeste niciun cadou (nu se genereaza nimic in plus). Comanda comerciala a
// pachetului Standard ("o singura melodie") ramane neschimbata — bonusul e livrat DUPA plata,
// ca surpriza/cadou, niciodata promis la checkout (vezi comanda.html, neschimbat).
//
// HOTFIX 2026-08-08 (fluxul de editare cu alegere Standard), context istoric — vezi mai sus
// corectia care il inlocuieste: pana la 2026-09-13, Standard NU livra niciodata o "melodie
// cadou". De cand editarea Standard poate pastra ambele variante
...
// checkout). CORECȚIE (2026-09-13): Standard nu mai ramane fara aceasta facilitate — vezi
// corectia de la inceputul fisierului. O editare Standard produce acelasi tipar de 2 variante
// ca Video (originala + editata), si acum foloseste ACEEASI regula STRICTA ca Video mai jos.
//
...
function getGiftVariant(order) {
  ...
  if (order.plan === 'video' || order.plan === 'standard') {
...
// ==========================================================================================
// CERINTA (2026-09-13, runda 2, "a treia melodie cadou pentru Premium"): pe langa cele doua
// melodii CUMPARATE (selectedVariantId + selectedVariantId2, livrate ca "incluse in pachet" —
// vezi getGiftVariant de mai sus si succes.html), Premium livreaza acum si o A TREIA melodie,
// STRICT ca surpriza/cadou post-plata, aleasa dintre variantele NESELECTATE de client.
//
// Exemplu explicit din cerinta: sunt generate A/B/C/D (ambele melodii initiale au fost editate
// cel putin o data), clientul alege A si C pentru pachet -> bonusul e ales RANDOM din {B, D}.
// Daca a fost editata o singura melodie (3 variante reale in total), ramane STRICT o varianta
// nealeasa -> aceea e livrata direct, fara nicio alegere aleatoare. Daca nu a fost editata
// nicio melodie (2 variante reale, exact cele cumparate), nu exista nimic de mai livrat.
//
// pickPremiumBonusVariantId() e PURA (nicio scriere in DB) — alegerea REALA se face O SINGURA
// DATA, la confirmarea platii, in ACEEASI tranzactie atomica ce marcheaza comanda "ready" (vezi
// db.recordPaidOrderAtomically), si se persista in order.premiumBonusVariantId — niciodata
// recalculata la fiecare vizualizare a paginii (altfel clientul ar vedea o alta melodie cadou
// la fiecare reincarcare, sau — mai grav — bonusul livrat efectiv prin /media/full/:id/bonus
// nu ar mai corespunde cu ce a fost aratat anterior). rng e injectabil STRICT pentru teste
// deterministe (implicit Math.random in productie).
function pickPremiumBonusVariantId(order, rng = Math.random) {
  if (!order || order.plan !== 'premium') return null;
  const variants = order.variants || [];
  const candidates = variants.filter(v => v.id !== order.selectedVariantId && v.id !== order.selectedVariantId2);
  if (candidates.length === 0) return null;
  const idx = Math.min(candidates.length - 1, Math.floor(rng() * candidates.length));
  return candidates[idx].id;
}

// Rezolva id-ul PERSISTAT (order.premiumBonusVariantId) catre obiectul-varianta complet —
// STRICT lectura, nicio alegere/randomizare aici (asta se intampla o singura data, vezi mai
// sus). Null pentru orice pachet diferit de Premium sau daca nu exista niciun bonus (comanda
// Premium fara nicio editare — nimic ramas de livrat in plus fata de cele 2 cumparate).
function getPremiumBonusVariant(order) {
  if (!order || order.plan !== 'premium' || !order.premiumBonusVariantId) return null;
  return (order.variants || []).find(v => v.id === order.premiumBonusVariantId) || null;
}

module.exports = { getGiftVariant, pickPremiumBonusVariantId, getPremiumBonusVariant };

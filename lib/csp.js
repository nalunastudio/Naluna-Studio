...
//
// Admin (2026-09-18, reorganizare Dashboard): panoul, fost o singura pagina cu <script> inline
// (private/admin.html, care AVEA nevoie de un hash inregistrat manual mai jos — vezi istoricul
// git), e acum un set de pagini sub private/admin/*.html, FIECARE cu JS extern (<script
// src="/admin/*.js">), niciodata inline. script-src 'self' le permite deja, fara niciun hash de
// intretinut — de-aceea nu mai exista nicio inregistrare speciala pentru /admin* aici.
const fs = require('fs');
...
function buildCspDirectives() {
  ...
  return {
    ...
    directives: {
      defaultSrc: ["'self'"],
      // META PIXEL (2026-09-24, gasit prin testare REALA in productie — fbevents.js era blocat,
      // "violates ... script-src"): connect.facebook.net e STRICT gazda care serveste scriptul de
      // baza al Pixel-ului (vezi loadMetaPixelIfNeeded, public/js/analytics.js — script.src =
      // 'https://connect.facebook.net/en_US/fbevents.js') — NU www.facebook.com (acela e folosit
      // STRICT pentru trimiterea evenimentelor, vezi connectSrc/imgSrc mai jos, nu pentru a servi
      // cod JS). Un singur origin exact, fara wildcard.
      scriptSrc: ["'self'", 'https://cdnjs.cloudflare.com', 'https://www.googletagmanager.com', 'https://connect.facebook.net', (req) => scriptHashesForPath(req.path)],
      ...
      // https://www.facebook.com (2026-09-24, Meta Pixel): fbevents.js, o data incarcat, trimite
      // evenimentele (PageView/InitiateCheckout) STRICT catre www.facebook.com/tr — documentat de
      // Meta sa foloseasca, dupa caz, un beacon <img> (acoperit aici) SAU fetch/XHR/sendBeacon
      // (acoperit de connectSrc mai jos) — ambele directive necesare, niciuna suficienta singura.
      imgSrc: ["'self'", 'blob:', 'https://cdnjs.cloudflare.com', 'https://www.facebook.com', ...storageHosts],
      mediaSrc: ["'self'", 'blob:', ...storageHosts],
      connectSrc: ["'self'", ...storageHosts, 'https://www.google-analytics.com', 'https://www.googletagmanager.com', 'https://region1.google-analytics.com', 'https://www.facebook.com'],

...
// ADMIN_HTML_PATH: panoul de admin (/admin) e servit din afara lui PUBLIC_DIR (private/), deci
// bucla de mai jos care scaneaza PUBLIC_DIR nu il gasea niciodata — inainte de aceasta
// corectie, scriptSrc pentru ruta /admin nu avea NICIUN hash inregistrat, ceea ce ar fi blocat
// COMPLET scriptul inline al admin.html sub aceasta politica CSP (2026-09-17, descoperit la
// integrarea Social Media in Admin — vezi Etapa 4). Inregistrat explicit, separat de bucla
// PUBLIC_DIR, sub cheia '/admin' (calea reala a rutei, nu numele fisierului).
...
function loadPageScriptHashes() {
  ...
  try {
    ...
    map['/admin'] = hashInlineScripts(adminHtml);
  } catch (err) {
    map['/admin'] = [];
  }
...
function buildCspDirectives() {
  ...
  return {
    ...
    directives: {
      defaultSrc: ["'self'"],
      scriptSrc: ["'self'", 'https://cdnjs.cloudflare.com', 'https://www.googletagmanager.com', (req) => scriptHashesForPath(req.path)],
      ...
      connectSrc: ["'self'", ...storageHosts, 'https://www.google-analytics.com', 'https://www.googletagmanager.com', 'https://region1.google-analytics.com'],

...
//                             apelantul (server.js) a putut totusi calcula un semnal de
//                             vocal-onset independent (`vocalOnsetStartSeconds`) — folosit ca atare.
//   C. start_zero_fallback  — nicio informatie utila disponibila — previewStart = 0.
//
// `vocalOnsetStartSeconds`: semnal OPTIONAL calculat de apelant (server.js), NEATINS — nu e
// recalculat aici, doar folosit ca ultima plasa de siguranta inainte de 0.
// ==========================================================================================
  ...
  try {
    if (Array.isArray(alignedWords) && alignedWords.length > 0) {
      ...
      if (firstRealVocalStart !== null) {
        let previewStart = Math.max(0, firstRealVocalStart - VOCAL_LEAD_IN_SECONDS);
        if (durationValid && maxValid) {
          const maxStart = Math.max(0, durationSeconds - previewMaxSeconds);
          previewStart = clamp(previewStart, 0, maxStart);
        }
        return {
